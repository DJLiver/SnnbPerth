...
{
    partial class MetricAlarm
    {
        internal static void ProcessAlarm(MetricEvent t)
        {
            int c = GetCount(t);
            if (t.Status) //In alarm
            {
                if (c > 0)
                    UpdateAlarm(t);
                else
        ...
        internal static void DeleteAllAlarms()
        {
            using (var cdc = new SNDataClassesDataContext())
            {
                var a = from f in cdc.MetricAlarms
                        select f;
                cdc.MetricAlarms.DeleteAllOnSubmit(a);
                cdc.SubmitChanges();
            }
        }

        private static void RemoveAlarm(MetricEvent t)
        {
            using (var cdc = new SNDataClassesDataContext())
            {
                var a = from f in cdc.MetricAlarms
                        where f.Target == t.Target && f.Attribute == t.Attribute
                        select f;
                cdc.MetricAlarms.DeleteAllOnSubmit(a);
                cdc.SubmitChanges();
            }
        ...
        private static void UpdateAlarm(MetricEvent t)
        {
            using (var cdc = new SNDataClassesDataContext())
            {
                var a = (from f in cdc.MetricAlarms
                ...
                a.DateStamp = t.DateStamp;
                a.Message = t.Message;
                cdc.SubmitChanges();
            }
        }

        private static void AddAlarm(MetricEvent t)
        {
            using (var cdc = new SNDataClassesDataContext())
            {
                var ma = new MetricAlarm()
                {
                    Target = t.Target,
                ...
                cdc.MetricAlarms.InsertOnSubmit(ma);
                cdc.SubmitChanges();
            }
        }

        private static int GetCount(MetricEvent t)
        {
            using (var cdc = new SNDataClassesDataContext())
            {
                return (from f in cdc.MetricAlarms
                        where f.Target == t.Target && f.Attribute == t.Attribute
                        select f).Count();
            }
        }
    ...
    partial class FormatConfig
    {
        internal static List<FormatConfig> GetFormatConfigs( )
        {
            using (var cdc = new SNDataClassesDataContext())
            {
                var v = from f in cdc.FormatConfigs
    ...
    partial class RangeConfig
    {
        internal static List<RangeConfig> GetRangeConfigs(string unit )
        {
            using (var cdc = new SNDataClassesDataContext())
            {
                var v = from f in cdc.RangeConfigs
                        where f.Unit == unit
                        select f;
                return v.ToList();
            }
        }
        internal static void CreateRow(Target unit )
        {
            List<RangeConfig> rangeConfig = GetRangeConfigs("Default");
            using (var cdc = new SNDataClassesDataContext())
            {
                foreach (var defaults in rangeConfig)
                {

                    var v = from f in cdc.RangeConfigs
                            where f.Attr == defaults.Attr && f.Unit == unit.Name
                            select f;
                    if (v.Count() == 0)
                    {
                        RangeConfig rc = new RangeConfig()
                        {
                            ...
                            InRange = defaults.InRange
                        };
                        cdc.RangeConfigs.InsertOnSubmit(rc);
                        cdc.SubmitChanges();
                    }
                }
            }
    ...
    partial class MetricEvent
    {
        internal static void Add(MetricEvent ms)
        {
            using (var cdc = new SNDataClassesDataContext())
            {
                cdc.MetricEvents.InsertOnSubmit(ms);
                cdc.SubmitChanges();
            }
        }

        internal static void DeleteOldRows(int daysToKeep)
        {
            using (var cdc = new SNDataClassesDataContext())
            {
                var v = from f in cdc.MetricEvents
#if TEST
            where f.DateStamp < DateTime.UtcNow.Subtract(TimeSpan.FromMinutes(5))
#else
                        where f.DateStamp < DateTime.UtcNow.Subtract(TimeSpan.FromDays(daysToKeep))
#endif
                        select f;

                cdc.MetricEvents.DeleteAllOnSubmit(v);
                cdc.SubmitChanges();
            }
        }
    }
    ...
    partial class MetricCount
    {
        ...
        internal static void DeleteOldRows(int daysToKeep)
        {
            using (var cdc = new SNDataClassesDataContext())
            {
                var v = from f in cdc.MetricCounts
#if TEST
            where f.DateStamp < DateTime.UtcNow.Subtract(TimeSpan.FromMinutes(5))
#else
                        where f.DateStamp < DateTime.UtcNow.Subtract(TimeSpan.FromDays(daysToKeep))
                ...
                cdc.MetricCounts.DeleteAllOnSubmit(v);
                cdc.SubmitChanges();
            }
        }
    }
    ...
    partial class MetricConfig
    {
        ...
        internal static void DeleteOldRows(int daysToKeep)
        {
            using (var cdc = new SNDataClassesDataContext())
            {
                var v = from f in cdc.MetricConfigs
#if TEST
            where f.DateStamp < DateTime.UtcNow.Subtract(TimeSpan.FromMinutes(5))
#else
                        where f.DateStamp < DateTime.UtcNow.Subtract(TimeSpan.FromDays(daysToKeep))
                ...
                cdc.MetricConfigs.DeleteAllOnSubmit(v);
                cdc.SubmitChanges();
            }
        }
    ...
    {
        internal static List<DatabaseMaint> GetDB_Maint()
        {
            using (var cdc = new SNDataClassesDataContext())
            {
                return (from f in cdc.DatabaseMaints
                        select f).ToList();
             }
        }
    }
    ...
    partial class MetricMonitor
    {
        ...
        internal static void DeleteOldRows(int days)
        {
            using (var cdc = new SNDataClassesDataContext())
            {
